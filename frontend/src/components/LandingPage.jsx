...
export default function LandingPage({ onEnterApp }) {
  useEffect(() => {
    ...
    const lenis = new Lenis({
      duration: 1.6,       // Scrolling duration (higher is slower and smoother)
      easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t)), // Smooth exponential ease-out
      smoothWheel: true,
      wheelMultiplier: 0.95, // Gentle dampening multiplier
    });

    // Notify GSAP ScrollTrigger about scroll updates
  ...
  const navLogo = useMemo(() => (
    <div className="flex items-center space-x-2">
      <ShieldAlert className="h-4 w-4 text-[#FAF8F5] shrink-0" />
      <span className="text-[10px] font-black tracking-widest text-[#FAF8F5] font-mono leading-none">INDUSLINK</span>
    </div>
  ), []);

  const navItems = useMemo(() => [
    { label: 'The Gap', href: '#problem', onClick: () => scrollToSection("problem") },
    { label: 'The Shift', href: '#shift', onClick: () => scrollToSection("shift") },
    { label: 'The Loop', href: '#core-loop', onClick: () => scrollToSection("core-loop") },
    { label: 'Differences', href: '#differentiators', onClick: () => scrollToSection("differentiators") },
    { label: 'Architecture', href: '#architecture', onClick: () => scrollToSection("architecture") }
  ], []);
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] text-[#1D2A22] font-sans antialiased relative overflow-hidden selection:bg-[#1D3225] selection:text-white">
      ...
      {/* Floating Header Navigation Window */}
      <header className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-7xl bg-[#FAF8F5]/85 backdrop-blur-md border border-[#E3DFD5]/80 px-4 py-2.5 flex items-center justify-between rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] transition-all duration-300">
        <PillNav
          logo={navLogo}
          logoAlt="IndusLink Logo"
          items={navItems}
          activeHref=""
          baseColor="#1D3225"
          pillColor="#FAF8F5"
          pillTextColor="#1D3225"
          hoveredPillTextColor="#FAF8F5"
          onLaunchClick={onEnterApp}
        />
      </header>

      {/* 1. Hero Section with Large Rounded Curves and Forest Green Simulator Card */}
      <section className="relative pt-32 pb-28 px-6 bg-[#F6F4EE] rounded-b-[3.5rem] border-b border-[#E3DFD5] shadow-sm bg-grid-warm">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              ...
              >
                <span>SEE DIFFERENCES WE MAKE</span>
          ...
          <div className="lg:col-span-5 bg-[#F6F4EE] border border-[#E3DFD5] p-6 rounded-2xl shadow-sm relative">
            ...
            <div className="relative flex justify-center py-6">
              <svg width="300" height="160" viewBox="0 0 300 160" className="overflow-visible">
                {/* 1 Major Accident */}
                <polygon points="90,10 115,50 65,50" className="fill-red-500/10 stroke-red-500" strokeWidth="1.5" />
                {/* 30 Minor Incidents */}
                <polygon points="115,50 145,100 35,100 65,50" className="fill-amber-500/10 stroke-amber-500/50" strokeWidth="1.5" />
                {/* 300 Near Misses / Weak Signals */}
                <polygon points="145,100 175,150 5,150 35,100" className="fill-[#FAF8F5] stroke-[#E3DFD5]" strokeWidth="1.5" />
                
                {/* Numbers inside the pyramid */}
                <text x="90" y="38" className="fill-red-600 text-[11px] font-mono text-center font-bold" textAnchor="middle">1</text>
                <text x="90" y="82" className="fill-amber-600 text-[10px] font-mono text-center font-bold" textAnchor="middle">30</text>
                <text x="90" y="132" className="fill-[#3B4C41] text-[10px] font-mono text-center font-bold" textAnchor="middle">300</text>

                {/* Pointer lines */}
                <line x1="106" y1="34" x2="190" y2="34" className="stroke-red-500/40" strokeWidth="1" strokeDasharray="3,3" />
                <line x1="131" y1="78" x2="190" y2="78" className="stroke-amber-500/40" strokeWidth="1" strokeDasharray="3,3" />
                <line x1="161" y1="128" x2="190" y2="128" className="stroke-[#3B4C41]/30" strokeWidth="1" strokeDasharray="3,3" />

                {/* Labels on the right */}
                <text x="196" y="37" className="fill-red-600 text-[9px] font-mono font-bold">1 ACCIDENT</text>
                <text x="196" y="81" className="fill-amber-600 text-[9px] font-mono font-bold">30 MINOR EVENTS</text>
                <text x="196" y="131" className="fill-[#3B4C41] text-[9px] font-mono font-bold">300 WEAK SIGNALS</text>
              </svg>
            </div>
      ...
      <section id="differentiators" className="py-24 border-t border-[#E3DFD5] bg-[#FAF8F5] px-6 max-w-7xl mx-auto border-x border-[#E3DFD5]/70 bg-grid-warm">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          {/* Right Column: CardSwap Stack showcasing the 5 differentiators */}
          <div className="lg:col-span-8 flex justify-center items-center py-6 min-h-[440px]">
            <CardSwap
              width={560}
              height={320}
              cardDistance={16}
            ...
            >
              <Card className="bg-[#FAF8F5] border border-[#E3DFD5] p-6 text-[#1D2A22]">
                <div className="flex justify-between items-center text-[10px] font-mono text-[#3B4C41] border-b border-[#E3DFD5] pb-2 mb-3">
                ...
                <h4 className="text-lg font-black text-[#1D3225] uppercase tracking-tight font-mono">Scoring you can audit</h4>
                <p className="text-[#3B4C41] text-xs sm:text-sm leading-relaxed font-sans mt-3">
              ...
              <Card className="bg-[#FAF8F5] border border-[#E3DFD5] p-6 text-[#1D2A22]">
                ...
                <h4 className="text-lg font-black text-[#1D3225] uppercase tracking-tight font-mono">Actionable recommendations</h4>
                <p className="text-[#3B4C41] text-xs sm:text-sm leading-relaxed font-sans mt-3">
              ...
              <Card className="bg-[#FAF8F5] border border-[#E3DFD5] p-6 text-[#1D2A22]">
                ...
                <h4 className="text-lg font-black text-[#1D3225] uppercase tracking-tight font-mono">Dynamic risk ranking</h4>
                <p className="text-[#3B4C41] text-xs sm:text-sm leading-relaxed font-sans mt-3">
              ...
              <Card className="bg-[#FAF8F5] border border-[#E3DFD5] p-6 text-[#1D2A22]">
                ...
                <h4 className="text-lg font-black text-[#1D3225] uppercase tracking-tight font-mono">Complete audit trails</h4>
                <p className="text-[#3B4C41] text-xs sm:text-sm leading-relaxed font-sans mt-3">
              ...
              <Card className="bg-[#FAF8F5] border border-[#E3DFD5] p-6 text-[#1D2A22]">
                ...
                <h4 className="text-lg font-black text-[#1D3225] uppercase tracking-tight font-mono">Resilience in the field</h4>
                <p className="text-[#3B4C41] text-xs sm:text-sm leading-relaxed font-sans mt-3">
